...
1. **Data model** ([data-model.md](./data-model.md)) extracts entities from spec:
   ...
   - `ProviderResponse` — frozen dataclass at the SACP <-> adapter boundary. Existing type from `src/orchestrator/types.py`; relocates to `src/api_bridge/adapter.py` (canonical home). Fields: `model`, `content`, `prompt_tokens`, `completion_tokens`, `cost`, `latency_ms`, `provider_family`, `finish_reason`.
