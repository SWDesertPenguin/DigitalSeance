...
**Performance Goals**:
- Limiter middleware overhead per request: O(1) — exempt-path check, source-IP resolution, hash lookup on keyed-IP, lazy refill via timestamp delta, bucket increment/decrement. Captured on a sample of requests via `routing_log` middleware-duration row (spec 003 §FR-030).
- Per-IP-budget eviction at MAX_KEYS bound: O(1) amortized via `OrderedDict.popitem(last=False)` LRU.
- Audit-log coalescing flush: asynchronous background task; MUST NOT block request path. Flush cadence is once per minute per `(source_ip_keyed, minute)` bucket.
**Constraints**:
...
**Structure Decision**: Single Python service ("Option 1") consistent with the existing repo layout. New `src/middleware/` package isolates request-pipeline middleware from business logic. Audit coalescing lives under `src/audit/` because the per-(IP, minute) flush has its own lifecycle (background task) distinct from request handling. `src/main.py` gains only the registration call site; the middleware body stays in its module. No frontend changes — this is operator-facing infrastructure (V13 parallel to spec 016).
...
- `test_019_exempt_and_isolation.py` covers SC-004: §7.5 per-participant rate limiter behaves byte-identically with the network-layer limiter active. Drives a §7.5 contract probe under load and asserts no behavioral drift.
