...
- [ ] T052 [P] Phase-2 web-UI port-8751 forward note (NO CODE — comment only): add a docstring note in [src/middleware/network_rate_limit.py](../../src/middleware/network_rate_limit.py) referencing [spec.md "Assumptions"](./spec.md) — the middleware is process-wide, not per-port, so Phase-2 reuse on port 8751 is a wiring change in the Web UI app factory ([src/web_ui/app.py](../../src/web_ui/app.py)) and/or whatever process-wide registration site Phase 2 settles on, not a spec amendment. No additional task here
