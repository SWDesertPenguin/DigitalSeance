...
**As an** operator applying the refactor PR, **I want** the the deployment stack to pull AND both ASGI apps to come up successfully **so that** the rename is invisible to running clients and no log errors reference the old `mcp_server` path.
...
- **Given** `SACP_MCP_PROTOCOL_ENABLED=true` AND a valid bearer token, **When** an `mcp-remote` bridge invokes `initialize` against the MCP endpoint with a `protocolVersion` from the supported set (`{2025-03-26, 2025-06-18, 2025-11-25}`), **Then** the response MUST echo back that version AND carry a fresh `Mcp-Session-Id` header AND the server capabilities advertisement.
...
- **Phase 1 deployment-side `compose.yaml` update.** The repo-side `compose.yaml` is updated in the refactor PR; the running the deployment stack at `<deployment-stack-path>/compose.yaml` requires a separate manual update per the deployment runbook — repo-side changes do not propagate automatically.
...
- **FR-006**: The the deployment stack deployment config (repo-side `compose.yaml` + `.env`, plus any related scripts under `scripts/` or `docker/`) MUST be audited for `mcp_server` path references. Any hits MUST be updated. The operator MUST be informed via the PR description that the running-deployment file requires a separate manual update.
...
- **FR-015**: The `initialize` handshake MUST negotiate protocol version, exchange capability advertisements, and issue a fresh `Mcp-Session-Id` header. The server MUST support the version set `{2025-03-26, 2025-06-18, 2025-11-25}`. If the client's `protocolVersion` is in the supported set, the server MUST echo it back. If it is absent or outside the set, the server MUST return its preferred version (`2025-11-25`) per MCP lifecycle spec; the client decides whether to proceed. Only structurally invalid `protocolVersion` values (non-string, empty) are rejected with `-32602`. Amended 2026-05-15 — see appendix entry under Phase 2.
...
- **FR-024**: The discovery metadata endpoint `/.well-known/mcp-server` MUST be served on the same host:port as the MCP endpoint. The response MUST include the preferred protocol version (`protocol_version: 2025-11-25`), the full supported version set (`supported_protocol_versions: [...]`), auth scheme (bearer-token in v1), endpoint URL, and server name. The endpoint MUST be available even when `SACP_MCP_PROTOCOL_ENABLED=false`, responding with an `enabled: false` flag.
...
- **FR-091**: One OAuth subject (one human) MUST be able to participate in multiple SACP sessions concurrently. Each session has its own participant record; tokens issued for each are independent and isolated. Implemented via per-session token issuance: each authorization flow names a `session_id` (bound at `initialize` time), producing a JWT with a `session_id` claim per FR-080 that scopes that token exclusively to one session. A subject joining N sessions holds N independent tokens. (A2 analysis fix — mechanism made explicit.)
...
- **SC-009**: The deployment-side audit MUST document every `mcp_server` reference in `compose.yaml` + `.env` + any related script files and confirm each is updated. The operator MUST be informed via the PR description of the separate manual update needed for the running deployment.
...
- Q: Phase 2 MCP transport binding — port 8750 mount at `/mcp` vs. third ASGI app on a new port (e.g., 8752)? → A: Mount at `/mcp` on the existing port 8750 ASGI app. Reuses existing firewall + reverse-proxy + the deployment stack port mapping; aligns with FR-037's structural-separation stance (different protocol families on the same port); avoids a third ASGI app surface in `src/run_apps.py`. Locks FR-026.
...
- Q: Phase 1 deployment config audit scope? → A: Both repo-side `compose.yaml` AND running the deployment stack file at `<deployment-stack-path>/compose.yaml`. Repo-side updates in the refactor PR; operator applies the the deployment host update manually. (Locks FR-006.)
...
- Q: Phase 2 protocol-version negotiation behavior on mismatch? → A: Originally strict rejection (research.md §6). **Amended 2026-05-15:** the strict pin made the MCP layer unusable in practice — no shipping client (Claude Desktop, mcp-remote, claude code) negotiates the future-dated `2025-11-25` revision. The `initialize` handshake now supports the version set `{2025-03-26, 2025-06-18, 2025-11-25}`: known versions are echoed back; unknown or omitted versions get the preferred `2025-11-25` returned per MCP lifecycle spec, and the client decides whether to proceed. Only structurally invalid `protocolVersion` values are rejected with `-32602`. (Locks FR-015.)
...
- Deployment config path audit — repo-side + the deployment host file; operator applies the deployment host update (follow-up).
...
#### Phase 2
- Protocol version pin — supported set `{2025-03-26, 2025-06-18, 2025-11-25}`; preferred `2025-11-25` returned for unknown/omitted requests per MCP lifecycle spec. Original strict-rejection clarification reversed 2026-05-15 because no shipping client negotiates the future-dated revision (follow-up).
