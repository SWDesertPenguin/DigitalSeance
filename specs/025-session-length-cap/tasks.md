...
- [ ] T051 [US2] Implement `update_active_seconds(session, now)` in [src/orchestrator/length_cap.py](src/orchestrator/length_cap.py) — pure helper signature pending; runtime call-site wiring deferred to a follow-up commit since `start_loop` / `pause_loop` / `resume_loop` / `stop_loop` lifecycle endpoints need parallel updates and benefit from a contained PR
- [ ] T052 [US2] Add `active_seconds_accumulator` update call sites to [src/orchestrator/loop.py](src/orchestrator/loop.py) at every FSM transition — deferred (companion to T051)
