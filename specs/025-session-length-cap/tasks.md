...
- [ ] T052 [US2] Pause-aware accumulator updates at lifecycle transitions (`start_loop`/`pause_session`/`resume_session`/`stop_loop`) — deferred. The `effective_active_seconds` fallback already drives the canonical "set time cap, watch it fire" path; pause-time exclusion lands when this task ships.
...
- [ ] T093 Run quickstart.md walk-through end-to-end against a local Dockge stack — operator-driven validation; deferred until the deploy refresh
