...
### Session 2026-05-01 (audit fix/002-testability — Phase B)

- Q: Which 002 FRs lacked test coverage as of the pre-Phase-3 audit window? → A: Token expiry enforcement (FR-002, FR-012, FR-013), IP binding (FR-016, FR-017, FR-018), TRUST_PROXY handling (FR-023), facilitator transfer atomicity + audit log (FR-011, FR-014), and brute-force gap documented absence (FR-024). These gaps are closed by `tests/test_002_testability.py` (17 tests) landing in `fix/002-testability`. Four items remain `untested` with Phase B / Phase 3 triggers: FR-004 (token plaintext in logs — covered by 007 ScrubFilter; dedicated log-pattern test deferred Phase E), FR-015 + FR-021 (pending endpoint guard — deferred to fix/011-testability endpoint matrix), FR-022 (HTTPBearer malformed-header matrix — deferred Phase B), FR-020 (pending scope per-session override — Phase 3 feature). FR-to-test traceability table for 002 added to `docs/traceability/fr-to-test.md`.
