...
**Decision: DEFER FR-020 to a follow-up amendment.** The v1 implementation surface — deployment-owner authentication endpoint, `account_participants` row-repointing query, admin audit row schema, regular-account 403 contract — is achievable on the v1 schema (`account_participants` join with FK to `participants` supports row-repointing without further migration), but the **deployment-owner authentication surface does not exist as a coherent operator-facing API today.** Building it for one ownership-transfer endpoint is a transport-layer + auth-surface expansion beyond the user's stated brief ("deferred to Phase 4 federation if it complicates Phase 3").

**What is deferred**: the new endpoint `POST /tools/admin/account/transfer_participants`, the `test_023_ownership_transfer.py` test surface, and `src/accounts/ownership_transfer.py`. Spec 023's tasks.md drops US4 from v1 scope and cross-references this decision.

**What is NOT deferred**: the data-model support (`account_participants` join with FK preserves the row-repointing capability without further schema change), the `admin_audit_log` action name reservation (`account_ownership_transfer` is added to the contract anyway, so the future implementation can land without a contract amendment), the cross-spec dependency clarity (spec 024 facilitator-scratch does NOT consume FR-020, so its scheduling is unaffected).

**Rationale**: the deployment-owner authentication boundary is a new architectural surface — it touches operator-API authentication design (mTLS? Operator-scoped session tokens? Magic IPs? Filesystem access?), not just one endpoint's auth check. Consensus design choice for "who is the deployment owner and how do they authenticate to a runtime endpoint" warrants its own clarify cycle. Folding it into spec 023 expands the spec from "user accounts" to "user accounts + operator administrative API" — a scope the user explicitly said to defer if it complicates Phase 3.

**Alternatives considered**:
- **Ship FR-020 in v1 using a one-off env-var-based admin password** — rejected; the operator-API auth model deserves its own clarify, not a one-off shortcut. Locking in a one-off would create a recon surface (the env var path) and a future migration burden when the proper operator-auth lands.
- **Ship FR-020 with a literal "you must SSH into the host and run a CLI command" path** — rejected; this is a transport-layer change (CLI vs. HTTP endpoint), not just an auth-layer change. Adds surface.
- **Keep FR-020 in scope but mark it as "future-stub"** — rejected; partial implementation of an admin endpoint is worse than no implementation. Either it works or it doesn't ship.
