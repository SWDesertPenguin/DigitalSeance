...
- [ ] T037 [P] [US1] Acceptance scenarios 1-3 (account-creation, code-emission, verification, code-rejection on incorrect/expired) in [tests/test_023_account_create.py](./../../tests/test_023_account_create.py).
- [ ] T038 [P] [US1] Acceptance scenarios 4-5 (login success → cookie + audit row; login failure → generic `invalid_credentials` 401, no info leak between non-existent-email and wrong-password) in [tests/test_023_account_login.py](./../../tests/test_023_account_login.py).
- [ ] T039 [P] [US1] Acceptance scenario 6 + SC-006 (per-IP login rate limiter trips at threshold; 429 + `Retry-After`; mirrors spec 009 §FR-002 / FR-003 shape) in [tests/test_023_login_rate_limit.py](./../../tests/test_023_login_rate_limit.py).
- [ ] T040 [P] [US1] SC-005 timing-attack resistance: login with non-existent email vs. existing email + wrong password produce identical responses (HTTP 401 + identical body + identical timing within ±5ms) in [tests/test_023_login_timing.py](./../../tests/test_023_login_timing.py). Implementation pattern: always run argon2id verify against a pinned dummy hash on the email-miss path so timing is uniform.
- [ ] T041 [P] [US1] SC-007 transparent re-hash on parameter change in [tests/test_023_argon2id_rehash.py](./../../tests/test_023_argon2id_rehash.py): seed an account with a low-parameter argon2id hash; bump `SACP_PASSWORD_ARGON2_TIME_COST`; log in; assert the post-login hash uses the new parameters (`PasswordHasher.needs_rehash` returns False after the re-hash) and the `accounts.password_hash` row is updated.
- [ ] T042 [P] [US1] SC-012 ScrubFilter coverage in [tests/test_023_scrub_filter.py](./../../tests/test_023_scrub_filter.py): drive create + verify + login + change-password + email-change flows; assert no plaintext password, no plaintext code, and no email body content appears in any log line. Cross-ref spec 007 §FR-012.

### Implementation for User Story 1

- [ ] T043 [US1] Implement `Account` + `AccountParticipant` pydantic models in [src/models/account.py](./../../src/models/account.py) per [data-model.md "Entities"](./data-model.md): frozen dataclasses; field annotations match the alembic schema.
- [ ] T044 [US1] Implement `service.create_account(email, password)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/create"](./contracts/account-endpoints.md): validates email syntax, password length [12, 1024]; checks unique-active-or-pending email; hashes password via `PasswordHasher.hash`; writes `accounts` row with `status='pending_verification'`; emits verification code via the email transport; writes `account_create` + `account_verification_emitted` audit rows.
- [ ] T045 [US1] Implement `service.verify_account(account_id, code)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/verify"](./contracts/account-endpoints.md): looks up the unconsumed `account_verification_emitted` row by HMAC-hashed code; checks TTL; flips status to `active`; writes `account_verification_consumed` audit row.
- [ ] T046 [US1] Implement `service.login(email, password, ip)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/login"](./contracts/account-endpoints.md): runs `LoginRateLimiter.check(ip)` first; case-insensitive email lookup; ALWAYS runs `PasswordHasher.verify` (against a pinned dummy hash on email miss for timing-attack resistance per SC-005); on success, mints a SessionStore sid with `account_id` set and updates `last_login_at`; on `needs_rehash`, re-hashes the submitted plaintext and UPDATEs `accounts.password_hash` (SC-007); writes `account_login` (success) or `account_login_failed` (failure) audit row.
...
- [ ] T059 [P] [US3] Acceptance scenario 1 (email change — notify-old + verify-new flow per clarify Q11; email field does NOT update until code is submitted) in [tests/test_023_email_change.py](./../../tests/test_023_email_change.py).
- [ ] T060 [P] [US3] Acceptance scenarios 2-3 (password change with correct current pw → new hash stored, subsequent logins require new pw; with incorrect current pw → 401, new pw NOT stored) + clarify Q12 SessionStore invalidation in [tests/test_023_password_change.py](./../../tests/test_023_password_change.py): drive a password change with multiple active sids for the account; assert all other sids invalidated; assert the actor's current sid survives.
- [ ] T061 [P] [US3] Acceptance scenarios 4-5 (account deletion → debug-export emit + email/password_hash zeroing + status flip; deleted-account login attempt → same generic `invalid_credentials` 401 as non-existent email) in [tests/test_023_account_delete.py](./../../tests/test_023_account_delete.py).
- [ ] T062 [P] [US3] Acceptance scenario 6 + SC-009 grace-period semantics in [tests/test_023_grace_period.py](./../../tests/test_023_grace_period.py): delete an account; on day 1, attempt re-registration with the same email → reject; advance test clock past `SACP_ACCOUNT_DELETION_EMAIL_GRACE_DAYS`; attempt re-registration → succeed. The test reads the env-var value rather than hardcoding 7 days (per spec SC-009).

### Implementation for User Story 3

- [ ] T063 [US3] Implement `service.request_email_change(account_id, new_email)` + `service.confirm_email_change(account_id, code)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/email/change"](./contracts/account-endpoints.md) and clarify Q11: emits verification code to NEW + heads-up to OLD; persists the pending change in `admin_audit_log` (transient via the `account_email_change_emitted` event, payload includes the new_email); on confirm, atomically updates `accounts.email` and writes `account_email_change_consumed`.
- [ ] T064 [US3] Implement `service.change_password(account_id, current_pw, new_pw, current_sid)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/password/change"](./contracts/account-endpoints.md) and clarify Q12: verifies current pw with the timing-uniform pattern; argon2id-hashes new pw; calls `SessionStore.delete_other_sids_for_account(account_id, except_sid=current_sid)`; writes `account_password_change` audit row.
- [ ] T065 [US3] Implement `service.delete_account(account_id, current_pw)` in [src/accounts/service.py](./../../src/accounts/service.py) per [contracts/account-endpoints.md "POST /tools/account/delete"](./contracts/account-endpoints.md) and FR-012 + FR-013: verifies current pw; calls spec 010's debug-export internally + emits via the email transport (deletion proceeds even if export-email fails per spec edge case); zeroes email + password_hash; populates `deleted_at` + `email_grace_release_at` from `SACP_ACCOUNT_DELETION_EMAIL_GRACE_DAYS`; writes `account_delete` audit row.
- [ ] T066 [US3] Wire the four US3 endpoints in [src/web_ui/account_routes.py](./../../src/web_ui/account_routes.py) (replacing the Phase 3 stubs from T047): `POST /tools/account/{email/change,email/verify,password/change,delete}`. Each requires account-cookie auth; payload validation per [contracts/account-endpoints.md](./contracts/account-endpoints.md); audit-row emission per [contracts/audit-log-events.md](./contracts/audit-log-events.md).
- [ ] T067 [US3] ScrubFilter pattern additions in [src/security/scrubber.py](./../../src/security/scrubber.py) (or the equivalent path per spec 007): scrub patterns for argon2id plaintext passwords (incoming request bodies), 16-char base32 codes, and email body content per FR-014. Cross-ref [data-model.md "Cross-spec references"](./data-model.md) — Spec 007 ScrubFilter extension.

### Frontend (spec 011 amendment surfaces — auth gate, login/logout, post-login session list, account-settings panel)

- [ ] T068 [US3] Add the auth-gate region to [frontend/app.jsx](./../../frontend/app.jsx) per [research.md §11](./research.md) surface 1: replaces the current "paste a token" landing with a "log in or create account" choice. Token-paste remains available behind a "use a token instead" link for the master-switch-off operator deployment. Add region-marking comment `// region: auth-gate (spec 023)`.
- [ ] T069 [US3] Add the login/logout flow region to [frontend/app.jsx](./../../frontend/app.jsx) per surface 1+2: email + password form for login; "create account" form for new accounts; verification-code submission UI; logout button posting to a logout endpoint that invalidates the current sid. Add region-marking comment `// region: login-logout (spec 023)`.
- [ ] T070 [US3] Add the post-login session list region to [frontend/app.jsx](./../../frontend/app.jsx) per surface 3: renders `/me/sessions` response with `active_sessions` first (live click → POST `/me/sessions/{id}/rebind` → SPA navigates into session) and `archived_sessions` second (read-only-transcript click → opens existing archived-session view). Add region-marking comment `// region: post-login-session-list (spec 023)`.
- [ ] T071 [US3] Add the account-settings panel region to [frontend/app.jsx](./../../frontend/app.jsx) per surfaces 4-7: email change (with notify-old + verify-new UI flow), password change (current-pw + new-pw + confirmation; on success "session refreshed" toast since the actor's sid survives), account deletion (typed-confirmation modal; on submit shows post-deletion landing). Add region-marking comment `// region: account-settings (spec 023)`.
