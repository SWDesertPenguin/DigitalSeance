...
- [X] T025 [US1] Identify all existing `append_audit_event(...)` call sites in the codebase (grep for `append_audit_event`); for each call site that emits a facilitator-visible audit row (per the registry seed in research.md §9), add the `broadcast_session_id` argument so live audit events broadcast going forward
