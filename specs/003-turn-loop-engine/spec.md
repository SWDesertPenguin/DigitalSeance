...
- Q: SC-010 calls per-stage P95 budgets "enforceable contracts" while SC-011 calls per-turn end-to-end SLOs "aspirational" — these framings contradict each other. → A: Reconciled to a single framing: SC-010 budgets are **enforceable** (they reflect the orchestrator's own overhead, which is testable without external model dependency); SC-011 budgets are **enforceable** too, subject to model-class parameterization (P50 is expressed as "model-baseline class" rather than a fixed millisecond value, which is the correct framing for a multi-model system). The word "aspirational" has been removed from SC-011 and replaced with the enforceable framing: the SLO is a contract that operators are expected to monitor and alert on. Without telemetry to compute the rolling 30-day window, the contract is *observable* — the spec asserts observability, not that operators have already wired the dashboard.
- Q: Does this amendment change behavior? → A: No. Doc-consistency fix only.
- Q: FR-022 advisory-lock indefinite-block behavior is underspecified for the Phase 3 multi-instance scenario. → A: Added an explicit residual annotation to FR-022 and to the advisory-lock paragraph in Operations. Phase 3 multi-instance deployments carry a residual indefinite-block risk on the advisory lock; production deployments MUST monitor `routing_log.advisory_lock_wait_ms` (FR-032) and alert on sustained values > 100ms.
- Q: Does this amendment change behavior? → A: No. Doc-consistency fix only.
...
- **FR-022**: Concurrent turn writes MUST be serialized at the branch level via a transaction-scoped PostgreSQL advisory lock on `pg_advisory_xact_lock(hashtext(branch_id))` taken inside `MessageRepository.append_message`. The lock prevents `turn_number` collisions when an inject and an AI-turn persist race within the same branch (clarified 2026-04-15). The lock has no application-level timeout; lock contention blocks indefinitely until the holding transaction commits or aborts. Concurrent contention is bounded in practice because each transaction is short and the loop is single-threaded per session. **Phase 3 multi-instance residual**: when multiple orchestrator processes share the same session (Phase 3 scale-out), the advisory lock becomes a cross-process coordination point; lock-wait time is unbounded if a process holding the lock stalls or crashes without committing. Production deployments running Phase 3 multi-instance topology MUST monitor `routing_log.advisory_lock_wait_ms` (FR-032) and alert on sustained rolling-window mean > 100ms.
...
- **SC-010**: Per-stage P95 budgets (orchestrator overhead only; LLM dispatch_ms is model-dependent and excluded): routing ≤ 5ms [JUDGMENT], context_assembly ≤ 50ms [JUDGMENT], persist ≤ 20ms [JUDGMENT], post_pipeline ≤ 50ms (cross-ref 007 §FR-014 layer evaluation) [JUDGMENT]. Aggregate non-dispatch overhead P95 ≤ 125ms [JUDGMENT] — this is the SACP turn-loop tax operators can plan around independent of model choice. These are **enforceable** contracts: the spec author's engineering judgment on commodity hardware, testable in a benchmark fixture against the orchestrator container without a live LLM.
- **SC-011**: Per-turn end-to-end latency SLOs MUST be reported on a rolling 30-day window: P50 ≤ model-baseline (cloud Haiku class ~3s [JUDGMENT], local llama3.2:3b on CPU class ~30s [JUDGMENT]), P95 ≤ 2× P50, P99 ≤ FR-019 per-attempt timeout. Persistent P95 > 2× P50 indicates degradation worth root-cause investigation. These are **enforceable** SLOs: operators MUST monitor and alert on sustained P95 > 2× P50. The SLO is expressed as a model-class multiple (not a fixed millisecond value) because end-to-end latency is dominated by model dispatch time, which varies by provider class. Without telemetry infrastructure the contract is observable but not yet alarmed — wiring the dashboard is an operator responsibility, not a spec gap.
...
- **SC-010 routing ≤ 5ms** [JUDGMENT] — based on asyncio dispatch overhead on commodity hardware; not measured against production hardware at time of authoring.
- **SC-010 context_assembly ≤ 50ms** [JUDGMENT] — based on asyncpg query latency (LAN) + Python list-comprehension overhead for token budget accumulation.
- **SC-010 persist ≤ 20ms** [JUDGMENT] — single asyncpg INSERT on a co-located Postgres instance.
- **SC-010 post_pipeline ≤ 50ms** [JUDGMENT] — see 007 §SC-008; the 007 author's budget for the regex layers.
- **SC-011 cloud Haiku class ~3s P50** [JUDGMENT] — based on observed Anthropic Claude Haiku API latency on low-load sessions during Phase 1 shakedowns.
- **SC-011 local llama3.2:3b CPU class ~30s P50** [JUDGMENT] — measured during Phase 0 feasibility testing on a mid-range x86 host; varies significantly with host CPU class and concurrent load.
- **FR-019 per-turn timeout default 180s** [JUDGMENT] — the original 60s default proved insufficient for llama3.2:3b on CPU (migration 003 raised it); 180s is the empirically-derived floor for CPU-hosted models.
- **FR-031 compound-retry warn factor 2× (360s default)** [JUDGMENT] — operator-judgement threshold; not derived from measured retry distributions.
