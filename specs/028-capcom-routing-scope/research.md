...
**Decision**: The visibility filter runs as the **last** filter step in `ContextAssembler.assemble()` (`src/orchestrator/context.py`), AFTER all other priority-based message assembly steps but BEFORE the `_secure_content` / trust-tier wrap stage. Specifically, a new pure function `_filter_visibility(messages, participant, capcom_id) -> list[ContextMessage]` is invoked after `_add_messages` / `_add_history` complete, and its output replaces the message list before security wrapping. The filter consults the cached `capcom_participant_id` value resolved at assemble-start (one extra column on the `sessions` SELECT inside `assemble`).
