...
- [X] T057 Quickstart.md walk-through: operator workflow per [quickstart.md](./quickstart.md) Steps 1-6 (enable master switch → tune threshold → set session slider → set per-participant override → observe per-stage cost → disable/rollback). Run on a deployed orchestrator (the deployment stack)
