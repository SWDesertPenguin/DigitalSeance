...
### Session 2026-05-14 (FR-008 re-pipeline alignment)

- Q: FR-008's tail clause said "On facilitator approve / edit / reject, content is persisted verbatim per 007 §FR-005 — it does NOT re-enter the security pipeline." Is this correct? → A: No. The clause contradicted both Constitution §4.9 (approach (b), recorded 2026-05-01) and the very FR it cited (007 §FR-005). Both authorities require approved-or-edited review-gate drafts to re-enter the security pipeline before persisting; if the re-run still flags, the facilitator must supply an `override_reason` or reject the draft. Reject persists nothing. FR-008's tail clause has been rewritten to: "On facilitator approve / edit, content re-enters the security pipeline before persisting per 007 §FR-005 and Constitution §4.9; if the re-run still flags, the facilitator supplies an `override_reason` (logged to `security_events` with `layer='facilitator_override'`) or rejects the draft. On reject, nothing is persisted." Implementation in `src/orchestrator/loop.py` already follows the re-pipeline path per spec 012 FR-006; this amendment closes the spec-vs-implementation gap (`/speckit.analyze` finding 008-B1, 2026-05-13).
- Q: Does this amendment promote spec 008's Status away from "Implemented"? → A: No. The implementation has been correctly re-pipelining since spec 012 FR-006 landed (2026-05-01). This is a doc-consistency fix; no behavior change.
...
- **FR-008**: System MUST stage high-risk responses for facilitator review instead of persisting them. Staging is implemented as a draft row in the review-gate table (`_stage_for_review` in `src/orchestrator/loop.py`) plus the operator-notification surfaces defined in 007 §FR-016 (review-gate UI banner, `security_events` row, WS `routing_mode` event). On facilitator approve / edit, content re-enters the security pipeline before persisting per 007 §FR-005 and Constitution §4.9 approach (b); if the re-run still flags, the facilitator MUST supply an `override_reason` (logged to `security_events` with `layer='facilitator_override'` and `override_actor_id`) or reject the draft. On reject, nothing is persisted. Implementation follows spec 012 FR-006.
