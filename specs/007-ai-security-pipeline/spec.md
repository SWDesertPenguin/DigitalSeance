...
- Q: FR-010 reads as if canary leakage scanning is active ("scan responses for leakage"), but the actual scanning logic (the detect layer) is deferred per spec 008 FR-003.detect. → A: Added an explicit deferral note to FR-010. The canary embedding (three random tokens embedded in the system prompt at construction time) ships in Phase 1; the scanning of AI responses for those tokens is deferred to spec 008 FR-003.detect. The current Phase 1 implementation embeds the canaries but does not scan responses — this is a known partial implementation, not a silent gap.
...
- SC-009 purge wired in fix/007-security-events-purge (2026-05-05): `src/operations/retention_purge.py:purge_security_events(pool, retention_days)` deletes rows where `timestamp < NOW() - retention_days`. CLI entry point at `scripts/purge_security_events.py` reads `SACP_DATABASE_URL` and `SACP_SECURITY_EVENTS_RETENTION_DAYS` (default 90 days when unset, per SC-009). Operator schedules externally — cron / Ofelia / k8s CronJob; default recommended cadence is daily. The orchestrator does NOT auto-purge in-process; the script is one-shot. `validators.validate_security_events_retention_days` blocks startup on invalid values.
...
- **FR-010**: System MUST embed three random 16-char base32 canary tokens in system prompts. Multi-canary (vs single) is required per constitution §8 amendment — single-canary is insufficient because a single match disclosure reveals the canary structure, while three independent canaries make pattern-leak triangulation harder. **Deferral note**: canary leakage scanning (scanning AI responses for the embedded tokens) is deferred per spec 008 FR-003.detect — the response-scan layer ships when the detect layer of the security pipeline is implemented. Phase 1 embeds the canaries at prompt-construction time but does NOT actively scan responses for them; SC-006 is accordingly a Phase 3+ contract.
...
- **SC-006**: Canary token leakage detection is deferred to spec 008 FR-003.detect. Phase 1 embeds canaries but does not scan responses; this SC becomes enforceable when the detect layer ships. Placeholder retained for traceability.
