...
- Q: Does spec 030 add any new admin panel UI FRs to spec 011 (parallel to specs 022/024/025/026/027/029)? → A: No. Spec 030 Phase 2 (MCP protocol over Streamable HTTP), Phase 3 (tool surface mapping), and Phase 4 (OAuth 2.1 + PKCE) operate against the MCP endpoint on port 8750 and do not introduce SPA surfaces. The MCP authorization-server surface (`/oauth/authorize`, `/oauth/token`, `/oauth/cimd/{client_id}`, discovery metadata) is consumed by MCP clients (Claude Desktop, `mcp-remote`, etc.) — not by the Web UI. Phase 5 ships participant onboarding docs (`docs/participant-onboarding*.md`); the Web UI does not embed or link to these docs at the SPA level in this round.
- Q: How does spec 030's CSP `connect-src` interaction with the MCP origin reach spec 011? → A: The SR-001 `connect-src 'self' <SACP_WEB_UI_MCP_ORIGIN> <ws-equivalent> <SACP_WEB_UI_WS_ORIGIN>` directive already names the MCP origin via the existing `SACP_WEB_UI_MCP_ORIGIN` env var; spec 030's MCP protocol endpoint inherits this slot unchanged. No spec 011 SR amendment needed.
- Q: Does spec 011's "OAuth 2.1 with PKCE login flow (Phase 3)" Out-of-Scope entry require revision now that spec 030 Phase 4 ships OAuth? → A: No. The Out-of-Scope entry refers to spec 011's *own* SPA login flow (replacing the per-session static bearer behind the session cookie with an OAuth code-exchange flow). Spec 030 Phase 4 explicitly scopes OAuth to the *MCP endpoint*: per spec 030 §Overview, "the SACP participant API (the renamed `src/participant_api/` router surface, used by the Web UI) stays on static bearers indefinitely for backward compatibility." Spec 011's static-bearer-behind-session-cookie pattern is preserved. The Out-of-Scope line remains accurate.
- Q: Does this amendment promote spec 011's Status away from "Implemented"? → A: No. Spec 011's Phase 2 deliverables remain Implemented. Spec 030 contributes zero new FRs, zero new SCs, and zero new Implementation Phases subsections to spec 011. This amendment is a clarifications-only record of the Phase 1 rename's semantic preservation guarantee and the OAuth scope boundary.

### Session 2026-05-13 (spec 026 context-compression amendment)

- Q: Does spec 026 contribute any new FRs to the admin panel? → A: Yes. Spec 026 ships a "Compression Metrics" admin panel page (FR-060..FR-062) as Phase 3 deliverables. The page is gated by `SACP_COMPRESSION_PHASE2_ENABLED=true`; when the master switch is false the menu item is hidden and the route returns 404. The three FRs are tracked under spec 026's task list; they ship when spec 026 Phase 3 implements the metrics endpoint surface.
- Q: Does this amendment promote spec 011's Status away from "Implemented"? → A: No. Spec 011's Phase 2 deliverables remain Implemented. The three new FRs (FR-060..FR-062) are Phase 3 deliverables tracked under spec 026's task list; they ship when spec 026 Phase 3 implements the compression metrics endpoint. The `## Implementation Phases` section captures this split under the new "Phase 3e — Compression Metrics UI" subsection.

### Session 2026-05-12 (spec 024 facilitator-scratch amendment)

- Q: Where does the entry-point affordance for the scratch panel live in the SPA? → A: The session header surfaces a "Scratch" button alongside the existing admin-panel toggle. The button is gated by FR-009 role check (facilitator-only) AND by the spec 024 master switch (`SACP_SCRATCH_ENABLED=true`). When the master switch is off OR the participant is non-facilitator, the button MUST NOT render.
- Q: How does the panel render — modal, route, slide-over? → A: A slide-over panel from the session header, deep-linkable via the route `/session/:id/scratch` (matches spec 029's route-not-modal precedent for deep-linkability + bookmarkable URLs). The slide-over preserves the live transcript context alongside; closing the panel returns to the live view. Three tabs: Notes / Summaries / Review Gate. Tab state is local (no URL-query-param routing) so the URL stays stable while the user tab-switches.
- Q: How does autosave work in the Notes tab? → A: 2-second debounce on every keystroke; on save fire, the SPA POSTs to `PUT /tools/facilitator/scratch/notes/{id}` with the current version. Status indicator transitions through "saving..." → "saved" → "save failed" so the facilitator sees persistence state. Network blips buffer locally and retry on reconnect.
- Q: What does the promote-to-transcript modal look like? → A: A confirmation modal showing the EXACT text that will be injected, with Cancel + Confirm options. The Confirm button stays disabled while the note content is empty. On Confirm, the SPA POSTs to `/tools/facilitator/scratch/notes/{id}/promote`; on 200 the modal closes and the note row gains a "promoted on <ts>" indicator; on 409 (archived session) the modal shows an inline error explaining the archived-session restriction.
- Q: How does the scope chip in the panel header read? → A: A small chip in the panel header reads either "Scope: account (durable)" or "Scope: session (ephemeral on archive)". The chip is visible at all times the panel is open; the session-scoped variant carries a tooltip explaining that notes will be deleted when the session archives (clarify Q9 — no surprise loss).
- Q: How does the Review Gate tab reuse spec 029's DiffRenderer? → A: Per spec 029 contracts/shared-module-contracts.md §3 + §4: the tab imports the inline `DiffRenderer` React component from `frontend/app.jsx` and passes `(previousValue=<original draft>, newValue=<edited draft>, format="text")`. The threshold constants (`MAIN_THREAD_BYTE_THRESHOLD = 50_000`, `WORKER_BYTE_THRESHOLD = 500_000`) are inherited from `frontend/diff_engine.js` — spec 024 MUST NOT redeclare. The spec 029 FR-020 architectural test (extended for spec 024) enforces no parallel Myers-diff implementation.
- Q: What happens in the panel when the session is archived? → A: The scope chip and panel content render as before, but the promote-to-transcript button on every note row renders disabled with a tooltip "promote requires an active session". The Notes tab is read-only (editing is allowed on account-scoped notes for facilitator review purposes; the autosave PUT is a no-op for unchanged content). The Summaries + Review Gate tabs are read-only by definition.
- Q: Does this amendment promote spec 011's Status away from "Implemented"? → A: No. Spec 011's Phase 2 deliverables remain Implemented. The eight new FRs (FR-042..FR-049) are Phase 3 deliverables tracked under spec 024's task list; they ship when spec 024 implements its scratch endpoints, the facilitator_notes table, and the promote-to-transcript handler. The `## Implementation Phases` section captures this split.
...
- **FR-042** (Phase 3, ships with spec 024): The session header MUST surface a "Scratch" button gated by FR-009 role check AND by `SACP_SCRATCH_ENABLED=true` (spec 024 FR-019). The button navigates the SPA to the scratch slide-over panel at route `/session/:id/scratch`. When the master switch is `false`, the button MUST NOT render. When the participant is non-facilitator, the button MUST NOT render. Cross-ref spec 024 FR-024.
- **FR-043** (Phase 3, ships with spec 024): The scratch panel MUST render as a slide-over alongside the live transcript view at route `/session/:id/scratch` with three tabs (Notes / Summaries / Review Gate). The slide-over preserves the live-transcript context (per clarify Q1 — facilitator scratch-thinking requires live-transcript-context); closing the panel returns to the live view. Tab state is local (NOT URL-query-param routed) so the URL stays stable while the user tab-switches. The route is gated by FR-009 role check; non-facilitator navigation returns the same 403 surface as other facilitator-only routes.
- **FR-044** (Phase 3, ships with spec 024): The Notes tab MUST autosave on a 2-second debounce on every content change, POSTing `PUT /tools/facilitator/scratch/notes/{id}` with the current `version` per spec 024 FR-004. The save status indicator MUST transition through "saving..." → "saved" → "save failed" states. On HTTP 409 (stale version per spec 024 FR-004), the SPA MUST show a "note updated elsewhere — refresh to merge" indicator and offer a refetch + merge action. On network failure the SPA MUST buffer the change locally and retry on reconnect.
- **FR-045** (Phase 3, ships with spec 024): The promote-to-transcript affordance MUST render a confirmation modal showing the EXACT text that will be injected (per spec 024 FR-007). The Confirm button MUST be disabled when the note content is empty. On Confirm, the SPA POSTs to `/tools/facilitator/scratch/notes/{id}/promote` per spec 024 FR-006; on 200 the modal closes and the note row gains a "promoted on <ts>" indicator linking to the resulting transcript message; on 409 (archived session) the modal shows an inline error explaining the restriction; on 422 (empty content) the modal renders an error state.
- **FR-046** (Phase 3, ships with spec 024): The scratch panel MUST reuse the spec 029 shared-module contracts per `specs/029-audit-log-viewer/contracts/shared-module-contracts.md`: (a) the inline `DiffRenderer` React component from `frontend/app.jsx` for the Review Gate sub-panel; (b) the locked threshold constants from `frontend/diff_engine.js` (`MAIN_THREAD_BYTE_THRESHOLD = 50_000`, `WORKER_BYTE_THRESHOLD = 500_000`); (c) `format_label` / `formatLabel` for any audit-adjacent labels surfaced in the panel (e.g., the promote-action label shown in the note row's tooltip after promotion); (d) `format_iso` / `formatIso` for all timestamp rendering. Spec 024 MUST NOT redeclare any of these — the spec 029 FR-020 architectural test (extended for spec 024) enforces. Cross-ref spec 024 FR-026.
- **FR-047** (Phase 3, ships with spec 024): The Summaries tab MUST render the summary archive list at 20 entries per page (spec 024 FR-012) with offset pagination. Each row displays turn range + first 200 characters of narrative + click-to-expand. The expanded view renders the four structured sections (decisions / open_questions / key_positions / narrative per spec 005 §FR-005). A "Copy to notes" action selects text from the expanded view and creates a new note pre-populated with the copied text (the new note appears in the Notes tab). Pagination controls are a "Previous page" / "Next page" pair with a current-page indicator. Cross-ref spec 024 FR-011.
- **FR-048** (Phase 3, ships with spec 024): The Review Gate tab MUST render the review-gate history list (spec 024 FR-013) with one row per `review_gate_*` audit-log event. Each row displays timestamp (via `formatIso`), disposition (approved-verbatim / edit-and-approve / reject), and the participant id + display name (resolved via the existing audit-row JOIN). Click-to-expand for `review_gate_edit` rows passes `previous_value` + `new_value` into the inherited `DiffRenderer` component per FR-046; approve-verbatim renders the AI draft as-was without a diff; reject renders the rejected draft + the rejection reason recorded by the facilitator. Cross-ref spec 024 FR-014.
- **FR-049** (Phase 3, ships with spec 024): When the active session is archived, the scratch panel MUST render in an archived-session affordance state: (a) the scope chip remains visible; (b) the promote-to-transcript button on every note row renders disabled with tooltip "promote requires an active session" (matches spec 024 FR-006 HTTP 409); (c) the Summaries + Review Gate tabs remain browseable read-only by definition; (d) the Notes tab remains editable for account-scoped notes (the facilitator can still update their notes for review purposes; autosave PUTs are no-ops on unchanged content). Cross-ref spec 024 FR-016.
- **FR-060** (Phase 3, ships with spec 026): The admin panel MUST expose a "Compression Metrics" page accessible from the existing admin dashboard. The page MUST be gated by `SACP_COMPRESSION_PHASE2_ENABLED` — when the master switch is false the menu item is hidden and the route returns 404. The button is gated by FR-009 role check (facilitator-only). Cross-ref spec 026 FR-025.
- **FR-061** (Phase 3, ships with spec 026): The Compression Metrics page MUST display per-session: cache hit rate (hit count / total dispatches as a percentage, derived from `routing_log` rows with `reason='cache_hit'` or `reason='cache_miss'`), compression ratio (mean `output_tokens / source_tokens` across `compression_log` rows for the session), and information-density baseline plus count of flagged turns in the last 20 (derived from `convergence_log` rows with `tier='density_anomaly'`). Cross-ref spec 026 FR-007 + FR-019.
- **FR-062** (Phase 3, ships with spec 026): The Compression Metrics page MUST link from any session's compression ratio row to the underlying `compression_log` rows for that session, using the same detail-row drill-down pattern established by spec 022's detection-event history panel (FR-036 / FR-037). The drill-down view MUST display compressor_id, compressor_version, source_tokens, output_tokens, duration_ms, and created_at for each `compression_log` row. Cross-ref spec 022 FR-036.
- **FR-063** (Phase 3, ships with spec 028): Participant cards MUST render a CAPCOM badge when the participant's `routing_preference === 'capcom'`. The badge copy reads "CAPCOM" and is visually distinct from the existing routing-preference indicator. Gating: rendered to every participant in the session (NOT facilitator-only) so the human + every panel AI sees who currently mediates the visibility partition. Cross-ref spec 028 FR-004 + Clarifications Session 2026-05-14. The badge clears immediately on `capcom_disabled` / `capcom_rotated` / `capcom_departed_no_replacement` WS events.
- **FR-064** (Phase 3, ships with spec 028): Transcript messages MUST render a per-message visibility indicator distinguishing `visibility='capcom_only'` rows from `visibility='public'` rows. Suggested treatment: a small "Private to CAPCOM" pill at the row's leading edge plus a subtle row-background tint; exact visual settled at implementation. Indicator is visible to humans and the active CAPCOM AI (per spec 028 FR-006 the panel AI never receives capcom_only rows so the indicator never reaches their UI). Cross-ref spec 028 FR-001 + FR-006.
- **FR-065** (Phase 3, ships with spec 028): The facilitator admin panel MUST expose CAPCOM lifecycle controls gated by FR-009 role check AND by `SACP_CAPCOM_ENABLED=true` (spec 028 FR-021). The controls: an "Assign CAPCOM" picker offering every AI participant in the session; a "Rotate CAPCOM" picker offering AI participants OTHER than the active CAPCOM; a "Disable CAPCOM" button surfaced only when a CAPCOM is currently assigned. The three controls POST/DELETE against `POST /tools/session/capcom/assign`, `POST /tools/session/capcom/rotate`, `DELETE /tools/session/capcom` per spec 028 FR-007 / FR-008 / FR-009. On HTTP 409 (single-CAPCOM violation, no-CAPCOM-to-rotate) the SPA MUST surface a transient inline error without tearing down the panel. Cross-ref spec 028 FR-021. When the master switch is `false` the controls MUST NOT render.
- **FR-066** (Phase 3, ships with spec 028): The human composer MUST expose a per-message visibility toggle (`public` / `capcom_only`) when a CAPCOM is assigned for the session. The toggle's initial value follows `SACP_CAPCOM_DEFAULT_ON_HUMAN_JOIN` (read once at session-snapshot fetch time; the SPA derives the value from the session row). When no CAPCOM is assigned the toggle MUST NOT render (the only available scope is `public`). The toggle's selected value is sent as the `visibility` field on the `POST /tools/participant/inject_message` body per spec 028 FR-014 + FR-015 + FR-016. The HTTP 409 / 422 responses (no CAPCOM assigned / panel AI rejection) MUST surface as transient inline errors at the composer without dropping the user's draft. Cross-ref spec 028 FR-015 + FR-016 + Clarifications Q5 (capcom_query reply default is `capcom_only` irrespective of the env var).
- **FR-067** (Phase 3, ships with spec 021): The facilitator admin panel MUST expose a session-level register slider (1-5 positions: Direct / Conversational / Balanced / Technical / Academic) gated by FR-009 role check. Position 3 (Balanced) emits no tier-4 register delta — tier text alone, per spec 021 FR-007 / FR-013. The slider POSTs to `POST /tools/facilitator/set_session_register` with body `{slider_value: 1..5}` per spec 021's existing endpoint shape; on 200 the SPA updates local state optimistically. The slider's initial value is read from the `/me` response (`register_slider` field) at snapshot-fetch time. Slider deltas always emit regardless of `SACP_RESPONSE_SHAPING_ENABLED` per spec 021 edge case ("slider deltas always emit regardless of the master switch — the slider is a prompt composition concern, not a shaping concern"), so the control renders unconditionally for facilitators. Cross-ref spec 021 FR-007 / FR-008 / FR-010.
- **FR-068** (Phase 3, ships with spec 021): Each participant card MUST surface a per-participant register override widget gated by FR-009 role check (facilitator-only). The widget MUST distinguish three states: (a) no override active — display "register: <session-default-preset>" with a "Set override" affordance; (b) override active — display "register: <override-preset> (override)" with both "Change override" and "Clear override" affordances; (c) the underlying value is read from each participant's `/me`-equivalent payload (`register_slider`, `register_preset`, `register_source` where `register_source === 'participant_override'` distinguishes case b from a). Setting an override POSTs to `POST /tools/facilitator/set_participant_register_override` with body `{participant_id, slider_value}`; clearing POSTs to `POST /tools/facilitator/clear_participant_register_override` with body `{participant_id}` per spec 021's existing endpoints. Non-facilitators MUST NOT see the widget at all (per spec 021 FR-008's facilitator-only authority model). Cross-ref spec 021 FR-008.
- **FR-069** (Phase 3, ships with spec 021): The state_snapshot WebSocket payload MUST include the session's current `register_slider` value (in the session block) AND each participant's `register_slider` / `register_preset` / `register_source` triple (in the participants block). The SPA seeds FR-067 + FR-068 control state from this snapshot path; the existing `/me` endpoint (spec 021 FR-010) remains the canonical read for the caller's own effective register but is no longer required for AdminPanel render. WebSocket `participant_update` events (spec 002 FR-016) MUST include the three register fields so the SPA can reflect mid-session override changes without a refetch. Cross-ref spec 021 FR-010 + spec 002 FR-016.
...
- **SC-009** (Phase 3, ships with spec 026): The Compression Metrics page renders without error for a session with zero `compression_log` rows (empty-state renders all three metric panels with zero / N/A values and no JS errors).
- **SC-010** (Phase 3, ships with spec 026): The Compression Metrics page renders cache hit rate, compression ratio, and density baseline for a session with at least one `compression_log` row and at least one `routing_log` cache-marker row; all three metric values are non-null and match the values computed directly against the DB for the same session.
...
### Phase 3e -- Compression Metrics UI (ships with spec 026 Phase 3)
- "Compression Metrics" menu item in the facilitator admin dashboard gated by `SACP_COMPRESSION_PHASE2_ENABLED=true` and FR-009 role check (FR-060)
- Per-session metrics page: cache hit rate from routing_log cache markers, compression ratio from compression_log, density flagged-turn count from convergence_log (FR-061)
- Drill-down to per-session compression_log rows using the spec 022 detection-event detail-row pattern (FR-062)
- Empty-state rendering for sessions with zero compression_log rows (SC-009)
- Playwright e2e covering SC-009 and SC-010
