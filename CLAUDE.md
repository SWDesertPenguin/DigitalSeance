...
## Recent Changes
- Drop provider+model dedupe (2026-04-24) — `_reject_duplicate_ai` (facilitator) and `_reject_duplicate_ai_in_session` (sponsor) deleted. Two AIs of the same model under different API keys (multi-account, separate cost buckets, distinct personas) is a legitimate config; display_name uniqueness alone is sufficient to prevent UI ambiguity. Surfaced when a facilitator tried to add a second `anthropic/claude-haiku-4-5-20251001` with a different API key and got 409.
