...
"""Audit log tool happy-path + error-path tests. Spec 030 Phase 3, T095."""
...
def test_tools_list_includes_audit_log_tool(client) -> None:
    resp = client.post(
        "/mcp",
    ...
    assert "admin.get_audit_log" in names


def test_audit_log_tool_definition() -> None:
    from src.mcp_protocol.tools import REGISTRY

    entry = REGISTRY["admin.get_audit_log"]
    assert entry.definition.scopeRequirement == "facilitator"
    assert not entry.definition.aiAccessible
    assert entry.definition.paginationSupported


def test_audit_log_without_db(client) -> None:
    resp = client.post(
        "/mcp",
        json={
            "jsonrpc": "2.0",
            "method": "tools/call",
            "id": 2,
            "params": {"name": "admin.get_audit_log", "arguments": {}},
        },
        headers={"Authorization": "Bearer tok"},
    )
